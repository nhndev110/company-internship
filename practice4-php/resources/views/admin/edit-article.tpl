...
{block name='css'}
  ...
  <link rel="stylesheet"
    href="{assets path='assets/admin/css/pages/edit-article.css'}">
  <style>
...
{block name='js'}
  ...
  <script type="module"
    src="{assets path='assets/admin/js/pages/edit-article.js'}">
  </script>
{/block}

{block name='main'}
  <form id="editArticleForm" method="post" enctype="multipart/form-data">
    <input type="text" name="articleId" hidden value="{$article->getId()}">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <div class="float-sm-right">
              ...
              <button type="submit"
                class="btn-update-article btn btn-primary font-weight-bold">
                Update article
              </button>
            </div>
          </div>
              ...
              <div class="card-body">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control form-control-border"
                    ...
                    style="resize: vertical;">{$article->getDescription()}</textarea>
                </div>
                <div class="form-group">
                  <label>
                    Content
                  ...
                  <textarea id="articleContent"
                    name="articleContent">{$article->getContent()}</textarea>
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="form-group mt-3">
                  ...
                  <select id="articleTags" name="articleTags" multiple="multiple"
                    ...
                    {foreach from=$tags item=tag}
                      <option {foreach from=$article->getTagsId() item=articleTagId}
                          {if $tag->getId() == $articleTagId} selected {/if}
                        {/foreach} value="{$tag->getId()}">
                        {$tag->getName()}
                      </option>
                    {/foreach}
                  </select>
