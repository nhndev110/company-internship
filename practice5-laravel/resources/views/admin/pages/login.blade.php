...
<body class="hold-transition login-page">
  <div class="login-box">
    ...
    <div class="card card-outline card-primary">
      ...
      <div class="card-body">
        <p class="login-box-msg {{ empty(session('msg_err')) ? '' : 'text-danger' }}">
          {{ empty(session('msg_err')) ? 'Sign in to start your session' : session('msg_err') }}
        </p>
        ...
        <form action="" method="post" id="loginForm">
          ...
          <div class="input-group mb-3">
            <input type="text" class="form-control" placeholder="Enter Username" name="username" autocomplete="off"
              {{ empty(session('old_username')) ? 'autofocus' : '' }}
